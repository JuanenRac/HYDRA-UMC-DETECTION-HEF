...
- **New `registry add` subcommand** (`add_entry()`/`write_registry()`/
  ...
  against the real file). `registry add` hashes a real local `.hef` with
  the same `compute_sha256()` `verify_checksum()` itself uses, then
  validates the assembled entry through the exact same `_parse_entry()`
  gate `load_registry()` runs every entry through - no separate,
  potentially divergent rules. Rejects a duplicate `(name, version)` and
  the same path-traversal attempt `verify_checksum()` already rejects.
  Verified end to end against a real file, not just the underlying
  function: `registry add` followed by `registry validate` reports a
  real checksum match.
